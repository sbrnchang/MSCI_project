...
# each file runs for one winter (October 1st to 30th April) of the stated year
'''
def main(
    fb_path1 = "../carmen/daily_numpys/AK_CPOM/FB_interp_2016-2017_25km_20170403.npy",
    fb_path2 = "../carmen/daily_numpys/CS2_CPOM/FB_interp_2016-2017_25km_20170403.npy",
    verbose=False,
    ...
    for i in range (360):
        for j in range (360):
            if not np.isnan(ak[i][j]):
                ak_observations.append([grid_x[i][j]/1000000, grid_y[i][j]/1000000, 1, ak[i][j], 0.01])
    ...
    np.savetxt("observations.txt", observations_matrix_subset, '%g %g %d %g %g')
